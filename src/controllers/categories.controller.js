...
categoriesCtrl.renderCompleteCategorie = async (req, res) => {
    ...
    try{
        ...
        const categoriaEncontrada = await Categoria.findById(C_id);
        const cantidadPorCategoria = await AlimentoDeUsuario.countDocuments({U_id: U_id, C_id: C_id});
        //const alimentosPorCategorias = await AlimentoDeUsuario.find({U_id: U_id, C_id: C_id});


        const alimentosPorCategorias = await AlimentoDeUsuario.find({U_id: U_id, C_id: C_id}).sort({createdAt: 'desc'}).populate({ path: 'A_id', model: Alimento}).populate({ path: 'C_id', model: Categoria}).populate({ path: 'UN_id', model: Unidad}).populate({ path: 'M_id', model: Marca}).populate({ path: 'T_id', model: Tienda});


        res.render('categories/completecategorie', {categoriaEncontrada, cantidadPorCategoria, alimentosPorCategorias});
    }catch(e){
...
categoriesCtrl.renderUpdateFood = async (req, res) => {
    ...
    try{
        ...
        res.render('categories/edit', {alimentoDeUsuarioEncontrado, alimentos, categorias, unidades, marcas, tiendas, cantidad, diaFechaIngreso, mesFechaIngreso, anioFechaIngreso, diaFechaCaducidad, mesFechaCaducidad, anioFechaCaducidad});
...
categoriesCtrl.updateFood = async (req, res) => {
...
categoriesCtrl.deleteFood = async (req, res) => {
